...
for link in all_links:
    ...
    for i in range(0, len(details)):
        ...
        office_price = (
            details[i]
            .find(
                class_="css-ubngax-wrapperStyle-marginlessStyle-contentText1-baseOverridesStyle"
            )
            .get_text()
        )

        local_information = details[i].findAll(
            class_="css-jqw1nl-textStyle-contentText2"
        )

        title = soup.find(
            class_="css-1ky4310-headingStyle-titleText1-colorStyle"
        ).get_text()
