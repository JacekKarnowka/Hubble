...
def office_price_filtrating(df):
    ...
    df['Office price'] = df['Office price'].apply(
        lambda x: str(x).split("\n")[0].split(' ')[1][1:] if x != 'No information' else x)

    df['Office price'] = df['Office price'].apply(lambda x: str(x).replace(",", ""))
    df['Office price'] = df['Office price'].apply(lambda x: int(x) if x.isnumeric() else 'No information')
...
def cleaning_floor_column(df):
    ...
    df['Floor'] = df['Floor'].apply(lambda x: x if (str(x).split(' ')[0] == "Floor" or
                                                    str(x).split(' ')[-2] == "Floor") else 'No information')

    df['Floor'] = df['Floor'].apply(lambda x: str(x).split(' ')[:2] if str(x).split(' ')[0] == 'Floor' else x)
    df['Floor'] = df['Floor'].apply(lambda x: str(x).split(' ')[-2:] if str(x).split(' ')[-2] == 'Floor' else x)

    df['Building Floor'] = df['Floor']
    df.drop(labels=['Floor'], axis=1, inplace=True)

    df['Building Floor'] = df['Building Floor'].apply(lambda x: x if x[0] == "Floor" else 'No information')

    df['Building Floor'] = df['Building Floor'].apply(lambda x: x[1:] if x[0] == "Floor" else 'No information')

    df['Building Floor'] = df['Building Floor'].apply(lambda x: ' '.join([str(elem) for elem in x]))
    df['Building Floor'] = df['Building Floor'].apply(lambda x: x if len(str(x)) <= 2 else 'No information')
...
df['Min. Term'] = df['Min. Term'].apply(lambda x: str(x).split(" ")[2] if str(x).split(" ")[0] == 'Min'
                                                                        else "No information")

df['Min. Term'] = df['Min. Term'].apply(lambda x: int(x[0]) if str(x)[1] == 'm' else int(x[0]) * 12
                                                            if str(x)[1] == 'y' else x)
df.rename(columns={'Min. Term': 'Min. Term [months]'}, inplace=True)

df['Area'] = df['Up to [people]'].apply(lambda x: 'No information' if len(str(x).split('(')) == 1
                                        else (str(x).split('(')[1])[:-1])

df['Area'] = df['Area'].apply(lambda x: int(str(x).split(" ")[0]) if x != "No information" else x)
df.rename(columns={'Area': 'Area [sqft]'}, inplace=True)


df['Up to [people]'] = df['Up to [people]'].apply(lambda x: str(x).split('(')[0])
df['Up to [people]'] = df['Up to [people]'].apply(lambda x: x if len(str(x).split(" ")) < 3 
                                                  else str(x).split(" ")[2])
