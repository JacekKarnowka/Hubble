...
GRAPH_DIV_STYLE = {'display': 'inline-block',
                   ...
                   'height': '1500px',
                   'color': '#657895',
                   'background-color': RIGHT_MENU_COLOR
                   }

Graph_Height = 650
Graph_Width = 850
SLIDER_WIDTH = Graph_Width - 100


def get_coordinates(adress):
    payload = {'query': [adress], 'country': 'GB'}

    answer = requests.get('http://api.positionstack.com/v1/forward?access_key=5f5a76367052b2f26966faf4cd307507',
                          params=payload)
...
def generate_net(left_top, left_bottom, right_bottom, right_top):
    ...
    for j in range(0, const_value):
        for i in range(0, const_value):
            net_latitude.append([left_top[0][0] - j * Lat,
                                 left_top[0][0] - (j + 1) * Lat,
                                 left_top[0][0] - (j + 1) * Lat,
                                 left_top[0][0] - j * Lat])

            net_longitude.append([left_top[1][0] - i * Long,
                                  left_top[1][0] - i * Long,
...
def check_points(net, df):
    df_net = pd.DataFrame(columns=['left_top_lat', 'left_top_long', 'left_bottom_lat', 'left_bottom_long',
                                   'right_bottom_lat', 'right_bottom_long', 'right_top_lat', 'right_top_long'])
    ...
    for k in range(0, len(df_net['left_bottom_lat'])):
        list_how_many_buildings.append(len(df.loc[(df['Latitude'] > df_net.iloc[k]['left_bottom_lat']) &
        ...
        list_average.append((sum(df.loc[(df['Latitude'] > df_net.iloc[k]['left_bottom_lat']) &
                                        (df['Longitude'] > df_net.iloc[k]['left_bottom_long']) &
                                        (df['Latitude'] < df_net.iloc[k]['left_top_lat']) &
                                        (df['Longitude'] < df_net.iloc[k]['right_bottom_long']),
                                        'Price per person all options'])))

        list_how_many_locals.append((sum(df.loc[(df['Latitude'] > df_net.iloc[k]['left_bottom_lat']) &
                                                (df['Longitude'] > df_net.iloc[k]['left_bottom_long']) &
                                                (df['Latitude'] < df_net.iloc[k]['left_top_lat']) &
                                                (df['Longitude'] < df_net.iloc[k]['right_bottom_long']),
                                                'Option counter'])))
    ...
    df_net['Average price per person'] = df_net.apply(lambda row:
                                                      row['Average price per person'] / row['How many buildings']
                                                      if row['How many buildings'] != 0 else 0, axis=1)
...
def figure_1(center_lat=51.509865, center_long=-0.118092, zoom_lvl=9, max_price=100000, min_price=0):
    df_new_point = pd.DataFrame()
    df_new_point[['Title',
                  'Latitude',
                  'Longitude',
                  'Price per person all options']] = df2[['Title',
                                                          'Latitude',
                                                          'Longitude',
                                                          'Price per person all options']]

    df_new_point[['new_point_Latitude', 'new_point_Longitude']] = [center_lat, center_long]

    df_new_point['distance_between'] = df_new_point.apply(
        lambda row: math.dist([row['Latitude'], row['Longitude']],
                              [row['new_point_Latitude'], row['new_point_Longitude']]), axis=1)

    fig1 = go.Figure()

    if center_lat != 51.509865 and center_long != -0.118092:
        df_tmp = df_new_point.sort_values('distance_between', ascending=True).head(LOCALS_AVERAGE_PRICE)
        estimated_local_value = round(float(sum(df_tmp['Price per person all options'])) / LOCALS_AVERAGE_PRICE, 2)
        ...
        fig1.add_trace(go.Scattermapbox(
            ...
            hovertemplate='<b> Estimated local value per person: {} pound for month</b><br>'
                          '<b> Latitude:</b> {} <br>'
                          '<b> Longitude</b> {} <br>'.format(estimated_local_value,
                                                             center_lat,
                                                             center_long),
    ...
    df2['hoverText'] = 'text'
    df2['hoverText'] = df2.apply(lambda row: '<b> Name: {} </b> <br>'
    ...
    df2_range = df2[
        (df2['Price per person all options'] < max_price) & (df2['Price per person all options'] > min_price)]
    ...
    fig1.add_trace(go.Scattermapbox(
        ...
        marker=go.scattermapbox.Marker(
            colorbar={
                'title': {'text': 'Average price<br>per person<br>[pounds]',
                          'font': {'size': 15,
                                   'family': 'sans-serif'}
                          },
                'x': 1,
    ...
    fig1.update_layout(
        showlegend=False,
        hovermode='closest',
        title={'text': "Price per person by location (building)",
               'font': {'size': 20,
                        'color': 'black',
                        'family': 'sans-serif'},
               'x': 0.5},
...
def figure_2():
    ...
    for i in range(0, len(net[0])):
        if df_points['How many locals'][i] != 0:
            fig2.add_trace(go.Scattermapbox(
                ...
                mode='lines',
                fill="toself",
                fillcolor='rgba({},0,0,{})'.format(int(df_points['How many locals'][i] / 2 + 21),
    ...
    fig2.update_layout(
        showlegend=True,
        hovermode='closest',
        title={'text': "Density map- how many locals to rent",
               'font': {'size': 20,
                        'color': 'black'},
               'x': 0.5},
        font={'color': 'black',
...
def figure_3():
    ...
    for i in range(0, len(net[0])):
        if df_points['Average price per person'][i] != 0:
            fig3.add_trace(go.Scattermapbox(
                name='Price {}'.format(round(df_points['Average price per person'][i], 2)
                                       if df_points['Average price per person'][i] != 0 else 'No data'),
                lat=net[0][i],
                lon=net[1][i],
                mode='lines',
                fill="toself",
                fillcolor='rgba({},0,0,{})'.format(int(df_points['Average price per person'][i] / 3.5 - 20),
    ...
    fig3.update_layout(
        showlegend=True,
        hovermode='closest',
        title={'text': "Average price per person",
               'font': {'size': 20,
                        'color': 'black',
                        'family': 'sans-serif'
                        },
...
def get_distance(x1, y1, x2, y2):
    earthRadius = 6371
    dist = math.acos(math.sin(x2 * math.pi / 180.0) * math.sin(x1 * math.pi / 180.0) +
                     math.cos(x2 * math.pi / 180.0) * math.cos(x1 * math.pi / 180.0) *
                     math.cos((y1 - y2) * math.pi / 180.0)) * earthRadius
    return dist
...
def figure_4(min_people=0, max_people=300, names_list=None):
    ...
    df_fig_4['distance_from_center'] = df_fig_4.apply(lambda row: get_distance(51.516865, -0.118092,
                                                                               float(row['Latitude']),
                                                                               float(row['Longitude'])), axis=1)

    df_fig_4_range = df_fig_4[
        (df_fig_4['Up to [people]'] < max_people) & (df_fig_4['Up to [people]'] > min_people)]

    df_fig_4_range['color'] = df_fig_4_range['distance_from_center'].apply(lambda x:
                                                                           'rgb({},0,0)'.format(255 - int(x * 585)))
    ...
    fig4.add_trace(go.Scatter(
        ...
        mode='markers',
        marker={'color': df_fig_4_range['distance_from_center'],
                'size': 6,
                'opacity': 0.8,
                'showscale': True,
                'colorscale': 'ylorrd_r',
                'colorbar': {'title': {'text': 'Distance<br>[km]',
                                       'font': {'size': 15,
                                                'family': 'sans-serif'}
                                       },
                             'x': 1,
    ...
    fig4.update_layout(
        hovermode='closest',
        title={'text': "Price max people and distance from centrum",
               'font': {'size': 20,
                        'color': 'black'},
               'x': 0.5},
        font={'color': 'black',
    ...
    fig4.update_annotations(font={'size': 20,
                                  'family': 'sans-serif',
                                  'color': '#efb636'})
...
df2['Option sum price'] = df2['Title'].apply(lambda x: sum(df.loc[df['Title'] == x, 'Office price']))

df2['Option people sum'] = df2['Title'].apply(lambda x: sum(df.loc[df['Title'] == x, 'Up to [people]']))

df2['Price per person all options'] = df2.apply(
    lambda row: row['Option sum price'] / row['Option people sum'] if row['Up to [people]'] != 'NUN'
    else row['Option sum price'], axis=1)

df2['Price per person all options'] = df2['Price per person all options'].apply(lambda x: 1500 if x > 1500 else x)

df2['Price per person'] = df2.apply(
    lambda row: row['Office price'] / row['Up to [people]'] if row['Up to [people]'] != 'NUN'
    else row['Office price'], axis=1)
...
net = generate_net(left_top_corner, left_bottom_corner, right_bottom_corner, right_top_corner)
...
app.layout = html.Div([
    html.Div([
        html.Div(children=[
            ...
            html.H1(children='Locals to rent in London \n',
                    style={
                        'textAlign': 'center',
                        'color': '#efb636'
                    })
        ...
        html.H4('''Enter address to check 
                its estimated value:''',
    ...
    html.Div([
        ...
        html.Div([
            ...
            html.Div([
                html.Div([
                    dcc.RangeSlider(math.floor(min_office_price), max_office_price,
        ...
        html.Div([
            html.Div([
                dcc.Graph(id='fig_4',
                          figure=figure_4())], style={}),
            html.Div([
                dcc.RangeSlider(math.floor(min_office_full), max_office_full,
...
def update_output(n_clicks, value_slider, value):
    ...
    if n_clicks > 0:
        ...
        return figure_1(lat, long, 16, min_price=value_slider[0], max_price=value_slider[1])


@app.callback(
    Output('fig_4', 'figure'),
    [Input('price_full', 'value'),
     Input('fig_1', 'selectedData')])
